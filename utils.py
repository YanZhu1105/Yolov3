...
# prediciton = B, channel, height, width
def predict_transform(prediction, inp_dim, anchors, num_classes, CUDA = True):
    ...
    x_offset, y_offset = torch.FloatTensor(a).view(-1, 1), torch.FloatTensor(b).view(-1, 1)
    if CUDA: x_offset, y_offset = x_offset.cuda(), y_offset.cuda()
...
def letterbox_image(img, inp_dim):
    ...
    w, h = inp_dim
    new_w, new_h = int(img_w * min(w/img_w, h/img_h)), int(img_h * min(w/img_w, h/img_h))
