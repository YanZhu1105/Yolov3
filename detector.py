...
def arg_parse():
    ...
    parser.add_argument('--reso', dest = 'reso', help = 'Input resolution of the network. Increase to increase accuracy. Decrease to in crease speed',
...
if args.image_or_video == 'Image':
    ...
    for i, batch in enumerate(im_batches):
        ...
        if type(prediction) == int:
            for im_num, image in enumerate(imlist[i * batch_size: min((i + 1) * batch_size, len(im_batches))]):
    ...
    def write(x, results):
        c1, c2 = tuple(x[1:3].int()), tuple(x[3:5].int())
        img = results[int(x[0])]
        cls = int(x[-1])
        label = f'{classes[cls]}'
        cv2.rectangle(img, c1, c2, random.choice(colors), 1)
    ...
    #     cwd = Path(os.getcwd())
    #     det = Path('det')
    #     img_name = Path(os.path.split(img)[1])
    #     det_names.append(cwd.joinpath(det).joinpath(Path(img_name)))
    ...
    assert cap.isOpend, 'Cannot capture source'
